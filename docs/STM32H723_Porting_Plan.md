...
- **需移植或条件编译保留的插件（原 Core0 addon）**
  - 输入与逻辑类（在 `GP2040::setup()` 中加载、主循环 Preprocess/Process/Postprocess 中执行）：AnalogInput、MCP3208ADCAddon、LSM6DSRIMUAddon、LinearTriggerAddon、FourKeyTouchpadAddon、WiiExtensionInput、SNESpadInput、SliderSOCDInput、TiltInput、TG16padInput、ReverseInput、TurboInput、InputMacro。
...
- [ ] 设备端 HID 配置接口与协议实现（可在现有 Pico 分支上合并或通过编译选项启用）。
- [ ] 固件构建选项或分支：无 RNDIS、无 lwIP、无 httpd、不链接 `www/`，产出固件体积 **&lt;1MB**。
- [ ] PC 端 WebHID 配置工具或网页 + 使用与协议文档。
- [ ] **前端配置界面包含「报告率」设置项**：可选 2K、4K、8K，写入设备配置；文档说明含义与生效方式（为阶段 4 设备端实现 8K 能力及 2K/4K/8K 可配置提供配置来源）。
- [ ] 验收报告：固件体积、配置读写流程通过。
...
- [ ] `platform/stm32h723/`（或等价）下可编译、可烧录的工程。
- [ ] 顶层 CMake 或脚本支持 `-DPLATFORM=STM32H723` 并生成 stm32 目标。
- [ ] 简要说明：如何配置工具链、烧录方式、推荐 IDE/调试方式。
...
- [ ] `platform/` 下统一头文件及 STM32 实现，与现有 `#ifdef PLATFORM_PICO` 分支并存。
- [ ] 所有 `src/` 中直接引用 `hardware/*`、`pico/time.h` 等处改为通过平台层或条件编译后，STM32 目标能通过编译（可先不链接业务，仅验证平台层编译与链接）。
- [ ] 文档：GPIO/ADC/PWM 引脚与 STM32 外设的对应关系（或指向 BoardConfig 说明）。
- [ ] 时间层：按 5.2 节第 5 条及附录 A 实现 DWT 时间与睡眠 API。
...
- [ ] STM32 下 FlashPROM 可正常 load/save 配置，重启后配置保持。
- [ ] System 接口在 STM32 下编译通过且行为符合预期（无 BootMode 依赖）。
- [ ] 文档：Flash 分区说明（程序区 / 配置区 / 边界）。
...
- [ ] 单核主循环可编译、可运行；无 multicore、无 RNDIS、无 Web、无 NeoPico/Bootsel。
- [ ] 清单：已移除或条件编译的文件/符号列表，便于后续回溯。
...
- [ ] STM32 作为 USB 手柄被 PC/主机识别，至少一种模式（如 XInput）可正常识别并响应输入。
- [ ] **USB 具备 8K 回报率能力**；设备端支持从配置中读取 2K/4K/8K 设置并正确设置 bInterval（或等效）与主循环节奏。
- [ ] 文档：所用 USB 实例（建议 HS）、时钟源、DCD 文件清单；报告率配置项与 bInterval 对应关系。
...
- [ ] 第二 USB 作为 Host 可枚举 dongle，并完成至少一种模式的认证（如 XInput 或 PS4）。
- [ ] 文档：Host 栈选型、第二 USB 引脚与时钟、回调对接说明。
...
- [ ] STM32 固件可通过 HID 配置接口被 PC 端 WebHID 工具识别并完成配置读写；重启后配置生效。
- [ ] 文档（含与阶段 P 的差异说明，若有）。
...
- [ ] 多模式与 dongle 认证联调通过，无已知严重缺陷。
- [ ] 构建与烧录文档、WebHID 配置文档、发布清单。
